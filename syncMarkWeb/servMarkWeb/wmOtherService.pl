...
sub extract_data_from_buffer
{
	...
	for my $line (split (/\n/, $buffer)) {
		...
		next if not defined $bmSegments[$link];
                ...
                $webMarks{$agg_key}->{$ATTR{LINK}} = $bmSegments[$link] if $bmSegments[$link];

                $webMarks{$agg_key}->{$ATTR{DATE}} = $bmSegments[$date] if $bmSegments[$date];

                $webMarks{$agg_key}->{$ATTR{TEXT}} = $bmSegments[$text] if $bmSegments[$text]; 

		#$webMarks{$bmSegments[$link]}->{$ATTR{ID}} = $bmSegments[$id] if  $bmSegments[$id];

		$webMarks{$agg_key}->{$ATTR{ID}} = $bmSegments[$id] ?  $bmSegments[$id] : " [] " ;
	}
